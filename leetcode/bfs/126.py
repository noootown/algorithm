...
class Solution:
  def findLadders(self, beginWord, endWord, wordList):
    ...
    self.ans, self.short, d = [], 0x7FFFFFFF, defaultdict(set)
    ...
    def dfs(b, w):
      if b[-1] == endWord:
        if len(b) < self.short:
          self.ans = [b]
          self.short = len(b)
        elif len(b) == self.short:
          self.ans.append(b)
        return
      elif not w or len(b) > self.short:
        return

      s_ = w & d[b[-1]]
      if endWord in s_:
        dfs(b + [endWord], w - {endWord})
      else:
        for ww in w:
          dfs(b + [ww], w - {ww})

    dfs([beginWord], set(wordList) - {beginWord})
    return self.ans
...
class Solution:
  def findLadders(self, beginWord, endWord, wordList):
    ...
    if endWord not in wordList:
      ...
      return [[beginWord]]
    elif cal(beginWord, endWord):
      return [[beginWord, endWord]]

    def bfs(fpath, bpath):
      uni = set(fpath.keys()) & set(bpath.keys())
      if uni:
        ans = []
        for u in uni:
          for fp in fpath[u]:
            for bp in bpath[u]:
              ans.append(fp[:-1] + bp[::-1])
        return ans

      fpath_, bpath_ = defaultdict(list), defaultdict(list)
      for k, paths in fpath.items():
        for path in paths:
          for c in 'abcdefghijklmnopqrstuvwxyz':
            ...
            for index in range(len(word)):
              ...
              if neigh in ws:
                fpath_[neigh].append(path + [neigh])

      if not fpath_.keys():
        return []
      uni = set(fpath_.keys()) & set(bpath.keys())
      if uni:
        ans = []
        for u in uni:
          for fp in fpath_[u]:
        ...
        for path in paths:
          for c in 'abcdefghijklmnopqrstuvwxyz':
            ...
            for index in range(len(word)):
              neigh = word[:index] + c + word[index + 1:]
              if neigh in ws:
                bpath_[neigh].append(path + [neigh])
      if not bpath_.keys():
        return []

      return bfs(fpath_, bpath_)

    ws = set(wordList)
    fpath, bpath = defaultdict(list), defaultdict(list)
    fpath[beginWord].append([beginWord])
    bpath[endWord].append([endWord])
    return bfs(fpath, bpath)
...
class Solution:
  def findLadders(self, beginWord, endWord, wordList):
    ...
    def bfs_level(this_lev, oth_lev, tree, is_forw, words_set):
      if not this_lev:
        return False
      len(this_lev) > len(oth_lev) and bfs_level(oth_lev, this_lev, tree, not is_forw, words_set)
      words_set -= this_lev | oth_lev
        ...
        for c in 'abcdefghijklmnopqrstuvwxyz':
          for index in range(len(word)):
            ...
            if neigh in oth_lev:
              ...
              tree[word if is_forw else neigh].append(neigh if is_forw else word)
            if not done and neigh in words_set:
              next_lev.add(neigh)
              tree[word if is_forw else neigh].append(neigh if is_forw else word)
      done or bfs_level(next_lev, oth_lev, tree, is_forw, words_set)

    tree = defaultdict(list)
    bfs_level({beginWord}, {endWord}, tree, True, set(wordList))
